...
class blockbeamDynamics:
    ...
    def f(self, state, u):
        ...
        M = np.array([[self.m1,0],[0,(self.m2 * (self.ell**2))/3 + (self.m1 * z**2)]])
        C = np.array([[self.m1*z*thetadot**2 - self.m1*self.g*np.sin(theta)],
                      [(F*self.ell*np.cos(theta)) - (2.*self.m1*z*zdot*thetadot) - (self.m1*self.g*z*np.cos(theta)) - (((self.m2*self.g*self.ell)/2) * np.cos(theta))]])
